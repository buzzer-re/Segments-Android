...
module.exports = {
  ...
  pendingReceive: async function(wallet,frontiers) {
    ...
    if (pending.data.blocks[account].length <= 0) return false;
  ...
  pendingSend: async function(wallet, amount, to) {
      const account = wallet.getAccounts()[0].account;
      // process.exit();
  ...
  sendNano: async function(wallet, amount, to) {
    var amountRai = parseInt(amount * 1000000);
    var amountRaw = bigInt(amountRai).multiply("1000000000000000000000000");
    var balance   = wallet.getAccounts()[0].balance;
    if (amountRaw.greater(balance) || amount < 0)
      return false;

    // const frontierBlock = await this.loadChains(wallet).catch((err) => {
    //   console.log(err);
    // });
    const sendBlock = await this.pendingSend(wallet, amountRaw, to);

    await this.broadcastBlock(wallet, sendBlock);
  },
  getBalance: function(wallet) {
    var raw = wallet.getAccounts()[0].balance;
    var rai = raw.divide('1000000000000000000000000');
