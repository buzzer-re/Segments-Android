...
module.exports = function (password) {
  ...
  api.getBalanceUpToBlock = function (blockHash) {
    ...
    for (var _i = current.chain.length - 1; _i >= 0; _i--) {
      blk = current.chain[_i];
  ...
  api.addPendingReceiveBlock = function (sourceBlockHash, acc, from) {
    ...
    for (var _i5 in current.chain) {
      if (current.chain[_i5].getSource() == sourceBlockHash) return false;
    }

    var blk = new Block();
    if (current.lastPendingBlock.length == 64) blk.setReceiveParameters(current.lastPendingBlock, sourceBlockHash);else blk.setOpenParameters(sourceBlockHash, acc, raiwalletdotcomRepresentative);
  ...
  api.checkWork = function (hash, work) {
    ...
    blake.blake2bUpdate(context, (0, _functions.reverse_obj)((0, _functions.hex_uint8)(work)));
    blake.blake2bUpdate(context, (0, _functions.hex_uint8)(hash));
    var threshold = (0, _functions.reverse_obj) (blake.blake2bFinal(context));
    ...
    if (blk) {
      if (blk.ready()) {
        ...
        if (current.chain.length == 0) {
          // open block
          if (blk.getType() != 'open' && !lightWallet) throw "First block needs to be 'open'.";
  ...
  api.load = function (data) {
    ...
    if (decryptedBytes === false) throw "Wallet is corruped or has been tampered.";
    ...
    for (var _i7 in walletData.accounts || []) {
      var acc = walletData.accounts[_i7];
      switch (acc.type) {
        case KEY_TYPE.SEEDED:
          {
